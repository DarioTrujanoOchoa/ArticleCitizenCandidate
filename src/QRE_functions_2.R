...
ex_payoffs <- function(probs,p, q, FUN= PR){
  # expected payoffs for each option according to probs given
...
logit_response <- function(ex_payoffs,lambda){
...
library("nleqslv")
QRE_p <- function(lambda, probs, p, q, FUN=PR){ # encontramos el QRE
  x=nleqslv(x = probs,fn = belief_error_2x2x2, p=p,q=q, lambda=lambda, FUN=FUN)[[1]]
  return(x)
}

# -logL por juego 
neg_logL_2x2x2 <- function(data,trials, lambda, probs, p, q, FUN=PR){
...
# global analysis
neg_logL_global <- function(election_game, lambda, probs, p_HC, p_LC , q){
  global_sum <- neg_logL_2x2x2(election_game[1:3,1],election_game[4,1], lambda, probs, p_LC, q, FUN=PR)+
...
# neg_log likelihood by combinations of games G indicates which gamesto consider
...
#some plots
QRE_lambda_plot <- function(upper_lambda=.4, interval=.01, title = "QRE", probs, p, q, FUN){
  colores <- c("red", "blue", "green")
  plot(c(0,upper_lambda),c(0,1),col="white", 
       ylab="Participate Probability",xlab="Lambda", main=title)
  ...
  legend(x=0,y=1,legend = q, cex=0.8, col=colores,
          lty=1, title="Players")
}
